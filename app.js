...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function loadPublicMessages() {
    ...
    unsubscribeListener = onSnapshot(publicQuery, snapshot => {
      ...
      snapshot.forEach(doc => {
        messagesDiv.innerHTML += renderMessage(doc.data());
      });
  ...
  async function openChatById(chatId) {
    ...
    if (currentChatMeta?.members?.length > 2) {
      const owner = currentChatMeta.members[0];
  ...
  dmForm?.addEventListener("submit", async e => {
    ...
    if (members.length > 2) {
      ...
      await setDoc(doc(db, "privateMessages", chatId), { members, isGroup: true, name: groupName, updatedAt: serverTimestamp() }, { merge: true });
    } else {
  ...
  closeModal?.addEventListener("click", () => { youtubeModal.style.display = "none"; youtubeResults.innerHTML = ""; });
  ...
  onAuthStateChanged(auth, user => {
    if (user) {
      ...
      messageForm?.style.setProperty("display", "flex");
      dmForm?.style.setProperty("display", "flex");
      ...
      messageForm?.style.setProperty("display", "none");
      dmForm?.style.setProperty("display", "none");
  ...
  addPersonCancel?.addEventListener("click", () => { addPersonModal.style.display = "none"; });

  addPersonSearch?.addEventListener("input", async () => {
